    ...
    fn thread_pool(&mut self) -> &mut futures::executor::ThreadPool {
        if self.thread_pool.is_some() {
            ...
            let pool = match self.thread_count {
                Some(count) => futures::executor::ThreadPool::builder()
                    .pool_size(count)
                    .create(),
            ...
            self.thread_pool =
                Some(pool.unwrap_or_else(|err| panic!("failed to create job pool: {}", err)));
            self.thread_pool.as_mut().unwrap()
        }
    }

    pub fn execute<T: Send + 'static, E: Send + 'static>(
        &mut self,
        mut job: Job<T, E>,
    ) -> ExecutionResults<T, E> {
...
impl<T: Send, E: Send> Job<T, E> {
    ...
    pub fn add_task<
        N: Into<String>,
        F: FnOnce(&futures::task::Context) -> Result<T, E> + Send + 'static,
    >(
        &mut self,
        name: N,
        task: F,
